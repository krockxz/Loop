 ...
 *
 * Sticky navigation with theme toggle and auth modal.
 */

'use client';

import { motion, AnimatePresence } from 'motion/react';
import { Zap, Github, Menu, X, Mail } from 'lucide-react';
    ...
    <>
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <ThemeToggle className="text-sm text-muted-foreground hover:text-foreground transition-colors p-2" />
              <button
              ...
              <button
                onClick={openAuthModal}
                className="px-4 py-2 text-sm bg-primary hover:bg-primary/90 text-primary-foreground rounded-md font-medium transition-colors"
              >
                Get started
              </button>
            </div>
          ...
          {mobileOpen && (
            ...
            >
              <div className="px-6 py-4 space-y-4">
                ...
                <button
                  onClick={openAuthModal}
                  className="block text-left text-muted-foreground w-full"
                >
                  Sign in
                </button>
                <button
                  onClick={openAuthModal}
                  className="w-full px-4 py-2 bg-primary text-primary-foreground rounded-md text-center font-medium"
                >
                  Get started
                </button>
              </div>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 mb-8">
                <button
                  ...
                  disabled={isLoading}
                  className={cn(
                    "flex aspect-square items-center justify-center rounded-2xl border border-border bg-card transition-all hover:scale-105 active:scale-95 disabled:opacity-50"
                  )}
                ...
                <div className="relative">
                  ...
                  >
                    {isLoading ? (
                      ...
                      <X className="h-4 w-4 rotate-[-45deg]" />
                    )}
                  </button>
                </div>
              </form>
      ...
      <div className="md:hidden fixed bottom-4 left-4 right-4 z-40">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 px-4 py-3 bg-background border border-border rounded-lg text-foreground text-sm font-medium shadow-lg"
        >
          <Github className="w-4 h-4" />
          Star on GitHub
        </a>
