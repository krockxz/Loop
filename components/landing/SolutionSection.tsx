/**
 * Solution Section - Theme-aware
 *
 * Simplified, focused explanation that adapts to light/dark theme.
 */
    ...
    <section ref={ref} className="py-20 px-6 bg-background">
      <div className={`max-w-6xl mx-auto ${isInView ? 'reveal visible' : 'reveal'}`}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <span className="inline-block px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-500 text-sm font-mono-display mb-4">
              The Solution
            </span>
          ...
          <div className="flex items-center justify-center gap-4 flex-wrap">
            {['Create', 'Assign', 'Notify', 'Track'].map((step, i) => (
              <div key={step} className="flex items-center">
                <div className="px-4 py-2 rounded bg-muted border border-border text-sky-500 font-mono-display text-sm">
                  {step}
                </div>
                {i < 3 && (
